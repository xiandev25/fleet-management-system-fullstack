    ...
    <header class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-2xl font-extrabold tracking-wider text-gray-50 uppercase">
          Fleet <span class="text-brand-accent-amber">Registry</span>
        </h1>
      ...
      <button 
        ...
        class="flex items-center gap-2 px-5 py-2.5 bg-brand-accent-amber hover:bg-brand-accent-amber-hover text-brand-asphalt font-bold text-sm uppercase tracking-wider rounded-xl transition-all shadow-md hover:shadow-brand-accent-amber-glow"
      >
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="border-b border-brand-asphalt-lighter bg-brand-asphalt/40">
              <th class="px-6 py-4 text-xs font-black tracking-widest text-gray-500 uppercase">License Plate</th>
              <th class="px-6 py-4 text-xs font-black tracking-widest text-gray-500 uppercase">Make & Model</th>
              <th class="px-6 py-4 text-xs font-black tracking-widest text-gray-500 uppercase">Year</th>
              <th class="px-6 py-4 text-xs font-black tracking-widest text-gray-500 uppercase">Mileage</th>
              <th class="px-6 py-4 text-xs font-black tracking-widest text-gray-500 uppercase">Status</th>
          ...
          <tbody>
            <tr 
              ...
              :key="vehicle.id"
              class="border-b border-brand-asphalt-lighter hover:bg-brand-asphalt/20 transition-colors"
            >
              <td class="px-6 py-4 font-bold text-brand-accent-amber">{{ vehicle.license_plate }}</td>
              <td class="px-6 py-4 font-semibold text-gray-200">{{ vehicle.brand }} {{ vehicle.model }}</td>
              <td class="px-6 py-4 text-gray-400 font-medium">{{ vehicle.manufacture_year }}</td>
              <td class="px-6 py-4 text-gray-400 font-medium">{{ vehicle.current_mileage.toLocaleString() }} KM</td>
              <td class="px-6 py-4">
                <span 
              ...
              <td class="px-6 py-4 text-right space-x-3">
                <button @click="openEditModal(vehicle)" class="text-brand-sign-blue hover:text-brand-sign-blue-hover transition-colors">
                ...
                <button @click="handleDelete(vehicle.id)" class="text-status-stop hover:text-red-400 transition-colors">
                  <svg class="w-5 h-5 inline-block" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
      ...
      <div v-if="totalCount > 0" class="px-6 py-4 border-t border-brand-asphalt-lighter bg-brand-asphalt/40 flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          <button 
            ...
            class="p-1.5 rounded-lg border border-brand-asphalt-lighter text-gray-400 disabled:opacity-50 hover:bg-brand-asphalt-lighter transition-colors"
          >
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-brand-asphalt/80 backdrop-blur-sm">
      <div class="bg-brand-asphalt-light border border-brand-asphalt-lighter rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div class="p-6 overflow-y-auto">
          <form id="vehicleForm" @submit.prevent="submitForm" class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <input v-model="formData.license_plate" required class="w-full px-3 py-2 bg-brand-asphalt border border-brand-asphalt-lighter rounded-xl text-gray-100 text-sm focus:outline-none focus:border-brand-accent-amber" placeholder="e.g. LT-123-XY" />
            </div>
            <!-- Immatriculation -->
            <div>
              <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1.5">Immatriculation No.</label>
              <input v-model="formData.immatriculation_number" class="w-full px-3 py-2 bg-brand-asphalt border border-brand-asphalt-lighter rounded-xl text-gray-100 text-sm focus:outline-none focus:border-brand-accent-amber" placeholder="Optional" />
            </div>
            <!-- Brand -->
            <div>
              <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1.5">Brand</label>
              <input v-model="formData.brand" required class="w-full px-3 py-2 bg-brand-asphalt border border-brand-asphalt-lighter rounded-xl text-gray-100 text-sm focus:outline-none focus:border-brand-accent-amber" placeholder="e.g. Toyota" />
            </div>
            <!-- Model -->
            <div>
              <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1.5">Model</label>
              <input v-model="formData.model" required class="w-full px-3 py-2 bg-brand-asphalt border border-brand-asphalt-lighter rounded-xl text-gray-100 text-sm focus:outline-none focus:border-brand-accent-amber" placeholder="e.g. Coaster" />
            </div>
            <!-- Year -->
            <div>
              <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1.5">Manufacture Year</label>
              <input v-model.number="formData.manufacture_year" type="number" required class="w-full px-3 py-2 bg-brand-asphalt border border-brand-asphalt-lighter rounded-xl text-gray-100 text-sm focus:outline-none focus:border-brand-accent-amber" placeholder="2018" />
            </div>
            <!-- Mileage -->
            <div>
              <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1.5">Current Mileage (KM)</label>
              <input v-model.number="formData.current_mileage" type="number" required class="w-full px-3 py-2 bg-brand-asphalt border border-brand-asphalt-lighter rounded-xl text-gray-100 text-sm focus:outline-none focus:border-brand-accent-amber" placeholder="0" />
            </div>
            <!-- Status -->
            <div class="md:col-span-2">
              <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1.5">Operational Status</label>
              <select v-model="formData.status" required class="w-full px-3 py-2 bg-brand-asphalt border border-brand-asphalt-lighter rounded-xl text-gray-100 text-sm focus:outline-none focus:border-brand-accent-amber appearance-none">
        ...
        <div class="px-6 py-4 border-t border-brand-asphalt-lighter bg-brand-asphalt/40 flex justify-end gap-3">
          ...
          <button form="vehicleForm" type="submit" class="px-5 py-2 bg-brand-accent-amber hover:bg-brand-accent-amber-hover text-brand-asphalt font-bold text-sm uppercase tracking-wider rounded-xl transition-all shadow-md">
...
const fetchVehicles = async (page = 1) => {
  loading.value = true
  try {
    const response = await fleetApi.getVehicles({ page })
    ...
    alert("Could not load fleet data.")
  } finally {
    loading.value = false
  }
}
...
const getStatusClass = (status) => {
  switch (status) {
    ...
    case 'IN_MAINTENANCE':
    case 'REFUELLING':
      return 'bg-brand-accent-amber/20 text-brand-accent-amber border border-brand-accent-amber/30'
...
const handleDelete = async (id) => {
  if (confirm("Are you sure you want to completely remove this vehicle from the registry?")) {
    try {
